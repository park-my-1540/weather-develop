...
export default function RecentList({ arr, updateFunc, btnDelete }) {
  return (
        <ul>
          <li>
            <a href="#none" onClick={updateFunc} className="city" data-city={typeof arr === "object" ? arr.city : arr}>
              <FontAwesomeIcon icon={faSearch} /> {typeof arr === "object" ? arr.city : arr}
            </a>
            <p>
              {arr.date && (<span>{arr.date}</span>)}
              <a href="#none" onClick={btnDelete} className="city" data-city={typeof arr === "object" ? arr.city : arr}>
